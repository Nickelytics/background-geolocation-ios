...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(nullable NSError *)error
{
    ...
    if (statusCode == 285)
    {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self->_delegate && [self->_delegate respondsToSelector:@selector(backgroundSyncRequestedAbortUpdates:)])
            {
                [self->_delegate backgroundSyncRequestedAbortUpdates:self];
            }
        });
    }
    ...
    {
        dispatch_async(dispatch_get_main_queue(), ^{
            if (self->_delegate && [self->_delegate respondsToSelector:@selector(backgroundSyncHttpAuthorizationUpdates:)])
            {
                [self->_delegate backgroundSyncHttpAuthorizationUpdates:self];
            }
        });
    }
}
